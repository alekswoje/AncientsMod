 ...
 * right after the handshake (see {@code PrisonsModChannel.sendFullbrightBlacklist}
...
public final class Fullbright {
    ...
    public static void logReceived(Set<String> worlds) {
        if (worlds.isEmpty()) {
            PrisonsMod.LOGGER.info("[fullbright] blacklist cleared by server");
        } else {
            PrisonsMod.LOGGER.info("[fullbright] blacklist from server ({} worlds): {}",
                    worlds.size(), worlds);
        }
    }
}
