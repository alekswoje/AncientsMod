...
/**
 * Decoded form of {@link Protocol#PKT_MINING_RUSH_PING}.
 *
 * <p>Wire format is byte-for-byte identical to {@link MeteorPingPayload}
 * (label, RGB, x/y/z, world name, lifetimeMs) — mining-rush pings reuse the
 * meteor beam renderer. It's a separate packet/payload so the client can gate
 * it behind its own toggle and the server can colour it per ore tier. All
 * fields are bounds-checked at decode time.
 */
public record MiningRushPingPayload(String label, int colorRgb,
                                    double x, double y, double z,
                                    String worldName, int lifetimeMs) {

    public static MiningRushPingPayload decode(PacketByteBuf buf) {
        ...
        return new MiningRushPingPayload(label, rgb, x, y, z, world, lifetime);
    }
}
