...
/**
 * Decoded form of {@link Protocol#PKT_TEAR_PING}.
 *
 * <p>Wire format is byte-for-byte identical to
 * {@link MeteoriteShowerPingPayload} (label, RGB, x/y/z, world name,
 * lifetimeMs) — tear pings reuse the same beam renderer. It's a separate
 * packet/payload so the client can gate it behind its own toggle and the server
 * can clear it early via {@link TearPingClearPayload}. All fields are
 * bounds-checked at decode time.
 */
public record TearPingPayload(String label, int colorRgb,
                              double x, double y, double z,
                              String worldName, int lifetimeMs) {

    public static TearPingPayload decode(PacketByteBuf buf) {
        ...
        return new TearPingPayload(label, rgb, x, y, z, world, lifetime);
    }
}
