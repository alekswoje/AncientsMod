...
public abstract class ItemTerminalScreen extends Screen {
     ...
     * name / lore line) as a styled {@link Text}. Lives in
     * {@link com.aleks.ancientsmod.client.LegacyText} now that the jewel
     * sockets and HUD need the same parse; kept here as a delegate because
     * subclasses call it.
     */
    protected static Text legacyText(String s) {
        return com.aleks.ancientsmod.client.LegacyText.parse(s);
    }
    ...
    private ItemStack resolveStack(PvBundlePayload.Slot slot, int displayedAmount) {
        ...
        return com.aleks.ancientsmod.client.IconResolver.resolve(slot.materialKey, Items.BARRIER, displayedAmount);
    }
