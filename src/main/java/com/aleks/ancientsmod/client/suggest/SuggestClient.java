package com.aleks.ancientsmod.client.suggest;
...
import com.aleks.ancientsmod.client.screen.SuggestScreen;
...
public final class SuggestClient {
    ...
    private static boolean onCommand(String command) {
        ...
        AncientsMod.LOGGER.info("[Suggest] /suggest intercepted, INTENT sent");
    ...
    public static void onOpen(SuggestOpenPayload p) {
        MinecraftClient.getInstance().execute(() -> {
            if (state != State.INTENT_SENT && state != State.IDLE) {
                AncientsMod.LOGGER.debug("[Suggest] OPEN ignored, state={}", state);
                return;
            }
            token = p.token;
            lastError = "";
