...
import com.aleks.ancientsmod.client.screen.LootTableDetailScreen;
import com.aleks.ancientsmod.client.screen.LootTablesScreen;
...
public final class LootClient {
    ...
    private static boolean onCommand(String command) {
        ...
        NetworkHandler.sendLootRequest();
        AncientsMod.LOGGER.info("[Loot] /loottables intercepted, snapshot requested");
    ...
    public static void onSnapshotChunk(int version, int index, int count, byte[] chunk) {
        ...
        try {
            ...
            AncientsMod.LOGGER.debug("[Loot] snapshot decode failed", t);
        }
    }
     ...
     * every server (re)connect (see {@code AncientsModClient} JOIN/DISCONNECT) so a
