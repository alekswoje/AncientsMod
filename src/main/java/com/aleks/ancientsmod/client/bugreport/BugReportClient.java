package com.aleks.ancientsmod.client.bugreport;
...
import com.aleks.ancientsmod.client.screen.BugReportScreen;
...
public final class BugReportClient {
    ...
    private static boolean onCommand(String command) {
        ...
        AncientsMod.LOGGER.info("[BugReport] /bugreport intercepted, INTENT sent");
    ...
    public static void onOpen(BugReportOpenPayload p) {
        MinecraftClient.getInstance().execute(() -> {
            if (state != State.INTENT_SENT && state != State.IDLE) {
                AncientsMod.LOGGER.debug("[BugReport] OPEN ignored, state={}", state);
                return;
            }
            token = p.token;
            sections = p.sections;
